...
blink_count = 0
frame_count = 0
last_yawn_time = 0 # Prevent double counting

# History for charts
ear_history = []
mar_history = []
alert_history = [] # timestamps of alerts
recent_ear_values = [] # For moving average

# Calibration
is_calibrating = False
calibration_frames = 0
base_ear = 0.28 # Default starting value
base_mar = 0.15
...
def calculate_fatigue_score(ear, mar, b_rate, p, y, r, y_count):
    # Use relative EAR based on calibration
    # Normal EAR is typically around base_ear. 
    # Fatigue starts when EAR drops significantly below base_ear for a period.
    
    # 1. EAR Score (Weighted 45%)
    # Only start penalizing if EAR is below 90% of base_ear
    ear_threshold = base_ear * 0.9
    if ear < ear_threshold:
        # Scale from ear_threshold down to EAR_THRESHOLD (0.22)
        ear_range = ear_threshold - EAR_THRESHOLD
        ear_score = min(1, (ear_threshold - ear) / ear_range) * 45
    else:
        ear_score = 0

    # 2. MAR Score (Weighted 15%)
    # Only penalize if MAR is significantly higher than base_mar
    mar_limit = 0.4
    if mar > mar_limit:
    ...
    # 3. Head Pose Score (Weighted 20%)
    # Drivers naturally move their heads. Only penalize large deviations.
    pose_dev = (abs(p) + abs(y) + abs(r))
    if pose_dev > 45: # Threshold for significant head tilt/turn
        pose_score = min(1, (pose_dev - 45) / 45) * 20
    else:
        pose_score = 0

    # 4. Yawn impact (Weighted 20%)
    # Cumulative but capped
    ...
    while True:
        ...
        if running:
            ...
            if left_eye and right_eye and mouth and face_rect:
                ...
                # Use moving average for EAR to stabilize
                ...
                if is_calibrating:
                    ...
                    # Accumulate for average - assuming user is looking normally
                    if calibration_frames == 1:
                        base_ear = current_ear
                    else:
                        base_ear = (base_ear * (calibration_frames - 1) + current_ear) / calibration_frames
                ...
                # Fatigue & AI Prediction
                approx_blink_rate = blink_count / (frame_count/20 + 1) 
                fatigue_status, _ = fatigue_predictor.predict([
                    current_ear, current_mar, approx_blink_rate, 
                    head_pose[0], head_pose[1], head_pose[2]
                ])
                fatigue_score = calculate_fatigue_score(
                    current_ear, current_mar, approx_blink_rate,
                    head_pose[0], head_pose[1], head_pose[2], total_yawns
                )

                # Visuals
                x1, y1, x2, y2 = face_rect
                color = (0, 255, 0)
                if fatigue_status == "Sleepy" or fatigue_score > 40: color = (0, 255, 255)
                if fatigue_status == "Highly Fatigued" or fatigue_score > 70: color = (0, 0, 255)
                
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                
                # Draw landmarks
                for p in left_eye + right_eye: cv2.circle(frame, p, 2, (0, 255, 0), -1)
                for p in mouth: cv2.circle(frame, p, 2, (255, 0, 0), -1)

                # Drowsiness Logic (Prolonged Eye Closure)
                ...
                if closed_count >= CLOSED_FRAMES:
                    ...
                    if not recorder.is_recording and recording_enabled:
                        ...
                        log_event("ALERT", "Drowsiness detected (Prolonged Eye Closure)")
                        if alarm_enabled: play_alarm() # LOUD ALARM for eyes
                        if voice_enabled: play_voice_alert("Warning! Driver appears drowsy. Please wake up!")

                # Yawning Logic (Improved State Machine)
                if current_mar > MAR_THRESHOLD:
                    yawn_count += 1
                    if yawn_count >= 5: # Show badge after a few frames of opening mouth
                        yawning = True
                    
                    if yawn_count >= YAWN_FRAMES:
                        current_time = time.time()
                        # Only count if at least 6 seconds have passed since the last count
                        # This covers the duration of one long yawn and a short recovery period
                        if current_time - last_yawn_time > 6.0:
                            with metrics_lock:
                                total_yawns += 1
                                last_yawn_time = current_time
                                log_event("INFO", f"Yawning detected (Total: {total_yawns})")
                                
                                # Escalate voice warnings based on yawn frequency
                                if voice_enabled:
                                    print(f"DEBUG: Yawn detected. Total yawns: {total_yawns}")
                                    if total_yawns == 3:
                                        play_voice_alert("Alert! You have yawned 3 times. Fatigue detected. Please stay focused.")
                                    elif total_yawns >= 5:
                                        print("DEBUG: Triggering Critical Yawn Alert")
                                        play_voice_alert("Critical Warning! High fatigue detected. Please stop and rest immediately.")
                else:
                    yawn_count = 0
                    yawning = False

                # On-screen Info
                cv2.putText(frame, f"Fatigue: {fatigue_status} ({fatigue_score}%)", (30, 40), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
                cv2.putText(frame, f"EAR: {current_ear:.2f}  MAR: {current_mar:.2f}", (30, 70), 
                ...
                if yawning:
                    cv2.putText(frame, "Yawning...", (w//2-80, h//2 + 50), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                
                if recorder.is_recording:
                    cv2.circle(frame, (w-30, 30), 10, (0, 0, 255), -1)
                    cv2.putText(frame, "REC", (w-80, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                    try:
            ...
            # Reset states when not running
            face_detected = False
            drowsy = False
            yawning = False
            stop_alarm()
            recorder.stop_recording()
            cv2.putText(frame, "System Standby", (30, 40), 
...
def get_recordings():
    ...
    # Filter for video files and sort by newest
...
def clear_logs():
    if os.path.exists(LOG_FILE):
        with open(LOG_FILE, "w", encoding="utf-8") as f:
            f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [SYSTEM] Logs cleared\n")
    return jsonify({"success": True})

@app.route("/reset", methods=["POST"])
def reset_metrics():
    global total_yawns, total_alerts, blink_count, ear_history, mar_history, alert_history
    with metrics_lock:
        total_yawns = 0
        total_alerts = 0
        blink_count = 0
